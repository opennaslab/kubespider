...
class PeriodManager:
    ...
    def create_or_update_task(
        ...
        with session_context() as session:
            if task_id:
                task = session.query(PeriodTask).filter_by(id=task_id).first()
                if not task:
    ...
    def delete_task(self, task: [int, PeriodTask]):
        with session_context() as session:
            task = session.query(PeriodTask).filter_by(id=task).first() if isinstance(task, int) else task
